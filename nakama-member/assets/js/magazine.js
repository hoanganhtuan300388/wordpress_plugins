...
function OnConfirm(page_link){
  var form = document.mainForm;
  var gid = form.set_lg_g_id.value;

  var cname = form.P_C_NAME.value;

  var value = form.P_C_EMAIL.value;
  var postid = form.postid.value;
  var urlcheck = form.urlcheck.value;
  var ifmGetData = getData;
  var ifmSetUrl;


  if(IsLengthB(form.G_NAME.value, 0, 200, '会社又は組織名')) return errProc(form.G_NAME);

  if(IsNull(form.P_C_NAME.value, '氏名')) return errProc(form.P_C_NAME);
  if(IsLengthB(form.P_C_NAME.value, 0, 200, '氏名')) return errProc(form.P_C_NAME);


  if(IsNull(form.P_C_KANA.value, '氏名フリガナ')) return errProc(form.P_C_KANA);
  if(IsLengthB(form.P_C_KANA.value, 0, 200, '氏名フリガナ')) return errProc(form.P_C_KANA);
  ...
  if(IsNull(form.P_C_EMAIL2.value, 'E-MAIL再入力')) return errProc(form.P_C_EMAIL);
  if(form.P_C_EMAIL.value != form.P_C_EMAIL2.value){
  ...
  ifmSetUrl = page_link+"func=registMail&gid=" + gid + "&cname=" + cname + "&value=" + value+"&postid="+postid;
...
function retSearchVal_CheckMailMagazine(retSearchVal){
  ...
  if (retSearchVal-0 == 1) {
    alert("入力されたE-MAILは既にメルマガ購読されています。");
    return errProc(form.P_C_EMAIL);
  }


  if (retSearchVal-0 == -2) {
    if (confirm("メルマガ会員登録します。\nよろしいですか？")) {
