...
function OnKeyDownLgGid(){
  if (event.keyCode == 0x0d) {
    // enter キーの場合
    ShowLowerGroupName(document.mainForm.M_LG_G_ID.value);
  }
}
...
function OnChangeLowerGroup(){
  if(document.mainForm.M_LG_ID != undefined){
    ...
    document.mainForm.M_LG_ID.value = document.mainForm.M_LG_G_ID_SEL.value;
    document.mainForm.M_LG_NAME.value = document.mainForm.M_LG_G_ID_SEL.options[document.mainForm.M_LG_G_ID_SEL.selectedIndex].text;
  }
}
...
function CheckInputData(){
  ...
  if (form.m_chg.value == "1") {
    ...
    if (form.useRegisteredGid.checked == false || form.useRegisteredGid.checked == undefined) {
      bCheckGdata = true;
    }
  }
  ...
  if (form.m_chg.value == "1") {
    ...
    if (form.useRegisteredPid.checked == false || form.useRegisteredPid.checked == undefined) {
      bCheckPdata = true;
    }
  }
...
// 辞書検索
function OnDic(dicName, eleName, post_id, page_link){
  buf = page_link+'dicName='+dicName+'&eleName='+eleName+'&post_id='+post_id;
  gToolWnd = open(buf,
            'DetailWnd',
