...
function OnConfirm(){
	...
	}else if(form.P_PASSWORD.value == "" && form.P_PASSWORD.type == "hidden"){
		
		OnBlurPid();
	}

	
	if(form.G_PASSWORD.value == "" && form.G_PASSWORD.type == "hidden"){
		
		OnBlurGid();
	}


	if(form.M_LG_G_ID.type == "hidden") form.M_LG_G_ID.value = "dmshibuya";
...
function OnZipCallbackP(zipcode, sta, addr1, addr2){
	var form = document.mainForm;
	form.P_C_POST_u.value =  zipcode.slice(0,3);
	form.P_C_POST_l.value = zipcode.slice(3,8);
...
function OnZipCallbackCl(zipcode, sta, addr1, addr2){
	var form = document.mainForm;
	form.M_CL_C_POST_u.value = zipcode.slice(0,3);
...
function OnZipCallbackCo(zipcode, sta, addr1, addr2){
	var form = document.mainForm;
	form.M_CO_C_POST_u.value = zipcode.slice(0,3);
	form.M_CO_C_POST_l.value = zipcode.slice(3,8);
...
function funcHiraganaToKatakana(obj){
	parent.retSearchVal_rsHiraganaToKatakana(obj.value, obj.name);
}
...
function chkMustInput(){
	...
	if(mainForm.G_BIZ_INFO != undefined){
		if(mainForm.G_BIZ_INFO.length == undefined){
			if(mainForm.G_BIZ_INFO.type != "hidden"){
				if(mainForm.G_BIZ_INFO.type != "select-one"){
					if(IsNull(mainForm.G_BIZ_INFO.value, "会員間の企業情報交換の設定")){
						mainForm.G_BIZ_INFO.select();
						mainForm.G_BIZ_INFO.focus();
						return false;
					}
				}else{
					if(IsNull(mainForm.G_BIZ_INFO.value, "会員間の企業情報交換の設定")){
				...
				if(mainForm.G_BIZ_INFO_1 != undefined){
					if(mainForm.G_BIZ_INFO_1.type != "hidden"){
						if(IsNull(mainForm.G_BIZ_INFO_1.value, "会員間の企業情報交換の設定")){
							mainForm.G_BIZ_INFO_1.select();
							mainForm.G_BIZ_INFO_1.focus();
							return false;
						}
					}
				}else{
					if(mainForm.G_BIZ_INFO_u != undefined){
						if(mainForm.G_BIZ_INFO_u.type != "hidden"){
							if(IsNull(mainForm.G_BIZ_INFO_u.value, "会員間の企業情報交換の設定")){
								mainForm.G_BIZ_INFO_u.select();
								mainForm.G_BIZ_INFO_u.focus();
								return false;
							}
						}
					}else{
						if(mainForm.G_BIZ_INFO_YEAR != undefined){
							if(mainForm.G_BIZ_INFO_YEAR.type != "hidden"){
								if(IsNull(mainForm.G_BIZ_INFO_YEAR.value, "会員間の企業情報交換の設定")){
									mainForm.G_BIZ_INFO_YEAR.select();
									mainForm.G_BIZ_INFO_YEAR.focus();
									return false;
								}
							}
						}else{
							if(mainForm.G_BIZ_INFO_YEAR != undefined){
								...
								if(mainForm.G_BIZ_INFO_SEL != undefined){
									if(mainForm.G_BIZ_INFO_SEL.type != "hidden"){
										if(IsNull(mainForm.G_BIZ_INFO_SEL.value, "会員間の企業情報交換の設定")){
											mainForm.G_BIZ_INFO_SEL.focus();
											return false;
										}
									}
								}
	...
	if(mainForm.G_KOUKAI_DETAIL != undefined){
		if(mainForm.G_KOUKAI_DETAIL.length == undefined){
			if(mainForm.G_KOUKAI_DETAIL.type != "hidden"){
				if(mainForm.G_KOUKAI_DETAIL.type != "select-one"){
					if(IsNull(mainForm.G_KOUKAI_DETAIL.value, "会員一覧からの詳細画面表示設定")){
						mainForm.G_KOUKAI_DETAIL.select();
						mainForm.G_KOUKAI_DETAIL.focus();
						return false;
					}
				}else{
					if(IsNull(mainForm.G_KOUKAI_DETAIL.value, "会員一覧からの詳細画面表示設定")){
				...
				if(mainForm.G_KOUKAI_DETAIL_1 != undefined){
					if(mainForm.G_KOUKAI_DETAIL_1.type != "hidden"){
						if(IsNull(mainForm.G_KOUKAI_DETAIL_1.value, "会員一覧からの詳細画面表示設定")){
							mainForm.G_KOUKAI_DETAIL_1.select();
							mainForm.G_KOUKAI_DETAIL_1.focus();
							return false;
						}
					}
				}else{
					if(mainForm.G_KOUKAI_DETAIL_u != undefined){
						if(mainForm.G_KOUKAI_DETAIL_u.type != "hidden"){
							if(IsNull(mainForm.G_KOUKAI_DETAIL_u.value, "会員一覧からの詳細画面表示設定")){
								mainForm.G_KOUKAI_DETAIL_u.select();
								mainForm.G_KOUKAI_DETAIL_u.focus();
								return false;
							}
						}
					}else{
						if(mainForm.G_KOUKAI_DETAIL_YEAR != undefined){
							if(mainForm.G_KOUKAI_DETAIL_YEAR.type != "hidden"){
								if(IsNull(mainForm.G_KOUKAI_DETAIL_YEAR.value, "会員一覧からの詳細画面表示設定")){
									mainForm.G_KOUKAI_DETAIL_YEAR.select();
									mainForm.G_KOUKAI_DETAIL_YEAR.focus();
									return false;
								}
							}
						}else{
							if(mainForm.G_KOUKAI_DETAIL_YEAR != undefined){
								...
								if(mainForm.G_KOUKAI_DETAIL_SEL != undefined){
									if(mainForm.G_KOUKAI_DETAIL_SEL.type != "hidden"){
										if(IsNull(mainForm.G_KOUKAI_DETAIL_SEL.value, "会員一覧からの詳細画面表示設定")){
											mainForm.G_KOUKAI_DETAIL_SEL.focus();
											return false;
										}
									}
								}
