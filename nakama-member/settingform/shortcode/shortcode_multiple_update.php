...
# なかま２のURL
const NEXT_URL = "https://dev.nakamacloud.com/nakama2/ApplyMember/ApplyUpdateMember";
...
if(!isset($_SESSION['arrSession'])){
  ...
  $g_id = $_SESSION['arrSession']->G_ID;  //⇒エンコードを行いパラメータを渡す
  ...
  ?>
    <script>
      //なかま２の画面に遷移する
      window.location = "<?php echo NEXT_URL ?>?tgId=<?php echo base64_encode($tg_id) ?>&gId=<?php echo base64_encode($g_id) ?>&pId=<?php echo base64_encode($p_id) ?>"
    </script>
  <?php
  }
