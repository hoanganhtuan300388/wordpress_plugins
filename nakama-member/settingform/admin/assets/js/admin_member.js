jQuery(document).ready(function($) {
  ...
  $(".member-setting-group-id").change(function(){
    var v = $(this).val();
    $(this).next().val("public_"+v);
  });
  $("#member-setting-group-id").change(function(){
    $("#member_meta_p_id").val("public_"+$(this).val());
  });
  ...
  var flg_tab_third = 0;
  var flg_tab_four = 0;
  $('#content_option').on('click', '.type_member ul#type_member > li span', function(e) {
    var className = $(this).attr("class");
    $("#content_option ul#type_member > li").removeClass("ui-tabs-active");
  ...
  $("#content_option ul#type_member > li span").click(function(e){
      var className = $(this).attr("class");
      $("#content_option ul#type_member > li").removeClass("ui-tabs-active");
      $(this).parent().addClass("ui-tabs-active");
      $(".content-in").css("display", "none");
...
function checkValidLG_ID() {
  var LG_ID = jQuery("#set_lg_g_id").val();
  var TG_ID = jQuery("input[name=member_meta_group_id]").val();
  jQuery.ajax({
    url: '../wp-admin/admin-ajax.php',
    type : "POST",
    data: {
      action: 'member_check_LG_ID',
      post_id: jQuery("input[name=postID]").val(),
      LG_ID: LG_ID,
      TG_ID: TG_ID
    },
    success: function(response) {
      ...
      if(response.data == false){
        ...
        jQuery("#err_lg_id").show();
      }
      else {
        jQuery("input#publish").prop("disabled", false);
        jQuery("#err_lg_id").hide();
      }
      if(LG_ID == ''){
        jQuery("input#publish").prop("disabled", false)
      }
