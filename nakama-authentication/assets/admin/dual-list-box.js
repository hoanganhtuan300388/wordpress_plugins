...
(function($) {
    ...
    function createDualListBox(options) {
        ...
        $(options.parentElement).addClass('row').append(
                (options.horizontal == false ? '   <div class="col-5-dual left">'+'<h4>'+options.titleLeft+'</h4>' : '   <div class="col-md-6">') + 
                ...
                '   </div>' +
                (options.horizontal == false ? createVerticalButtons(options.moveAllBtn, options.labelAdd, options.labelDel) : '') +
                (options.horizontal == false ? '   <div class="col-5-dual left">'+'<h4>'+options.titleRight+'</h4>' : '   <div class="col-md-6">') + 
    ...
    function createHorizontalButtons(number, copyAllBtn) {
        if (number == 1) {
            return (copyAllBtn ? '       <button type="button" class="btn btn-default atr" data-type="atr" style="margin-bottom: 5px;"></button>': '') +
                '       <button type="button" class="btn btn-default ' + (copyAllBtn ? 'pull-right col-md-6' : 'col-md-12') + ' str" data-type="str" style="margin-bottom: 5px;" disabled></button>';
        } else {
    ...
    function createVerticalButtons(copyAllBtn, labelAdd, labelDel) {
        return '   <div class="col-2-dual left center-block" style="margin-top: ' + (copyAllBtn ? '80px' : '130px') +'">' +
            '<ul><li>       <button type="button" class="btn btn-default col-md-8 col-md-offset-2 str" data-type="str" style="margin-bottom: 10px;" disabled>'+labelAdd+'</button></li>' +
            ' <li>      <button type="button" class="btn btn-default col-md-8 col-md-offset-2 stl" data-type="stl" style="margin-bottom: 10px;" disabled>'+labelDel+'</button></li>' +
            '   </ul></div>';
    }
    ...
    $.fn.sortOptions = function() {
        return this.each(function() {
            $(this).append($(this).find('option').remove().sort(function(a, b) {
                var at = $(a)[0].getAttribute("id"), bt = $(b)[0].getAttribute("id");
